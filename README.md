...
- **Interview Expert**：模拟面试闭环——出题→用户回答→评分→下一题→结束→总结复盘，支持语音输入与面试进度追踪
...
- **测试**: pytest + pytest-asyncio（55+ 测试用例）
...
| GET | `/agent/sessions` | 获取会话历史列表 |
| GET | `/agent/sessions/{id}/messages` | 获取指定会话的消息记录 |
...
```env
AGENT_RECENT_MESSAGE_COUNT=10     # 完整保留的最近消息条数
AGENT_SUMMARY_MAX_CHARS=120       # 摘要层每条消息截断字符数
AGENT_SUMMARY_TOKEN_BUDGET=800    # 摘要层 token 预算（0=不限）
```
...
- 分层裁剪（token 估算、摘要压缩、分层构建、跨轮次累积、边界条件）
...
- **可观测性增强**：统一日志打印 + 前端可观测性标签（缓存命中/降级/复用/面试进度等）
- **调试接口**：`/debug/runtime` 和 `/debug/session/{id}`，仅在 debug 模式下注册
- **全流式 SSE**：所有分析路径（简历/JD/QA/面试）统一为 SSE 流式输出
- **PostgreSQL 元数据**：知识库 chunk 元数据迁移到 PostgreSQL，支持自动迁移与索引压缩
- **回归测试体系**：55+ 测试用例覆盖基础流程、多轮对话、缓存、KB 降级、持久化、分层裁剪
